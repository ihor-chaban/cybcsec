...
CybCSecGUI::CybCSecGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new CybCSecBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
...
#ifdef Q_OS_WIN
    QFile html("C:/cybcsec/index.html");
#else
    QFile html("/opt/cybcsec/index.html");
#endif
...
void CybCSecGUI::pageLoaded(bool ok)
{
...
void CybCSecGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void CybCSecGUI::urlClicked(const QUrl & link)
{
...
void CybCSecGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/cybcsec"), tr("&About CybCSec"), this);
    aboutAction->setToolTip(tr("Show information about CybCSec"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for CybCSec"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/cybcsec"), tr("&Show / Hide"), this);
...
void CybCSecGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("CybCSec") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/cybcsec_testnet"));
            setWindowIcon(QIcon(":icons/cybcsec_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/cybcsec_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("CybCSec client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/cybcsec_testnet"));
...
void CybCSecGUI::setWalletModel(WalletModel *walletModel)
{
...
void CybCSecGUI::setMessageModel(MessageModel *messageModel)
{
...
void CybCSecGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("CybCSec client"));
    trayIcon->setIcon(QIcon(":/icons/cybcsec"));
...
#ifndef Q_OS_MAC
void CybCSecGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CybCSecGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to CybCSec network", "", count));
}

void CybCSecGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void CybCSecGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void CybCSecGUI::changeEvent(QEvent *e)
{
...
void CybCSecGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void CybCSecGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::XCS, nFeeRequired));
...
void CybCSecGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void CybCSecGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void CybCSecGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void CybCSecGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CybCSecGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void CybCSecGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid CybCSec address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void CybCSecGUI::handleURI(QString strURI)
{
...
void CybCSecGUI::setEncryptionStatus(int status)
{
...
void CybCSecGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void CybCSecGUI::changePassphrase()
{
...
void CybCSecGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void CybCSecGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CybCSecGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void CybCSecGUI::updateStakingIcon()
{
...
void CybCSecGUI::detectShutdown()
{
